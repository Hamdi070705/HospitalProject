...
@section('content')
    ...
    <div class="min-h-screen bg-gray-50 py-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="fade-in">
                <h1 class="text-3xl font-bold text-txt mb-8 border-b pb-2">
                    Rekam Medis Anda
                </h1>
                ...
                @if ($servicesGroups->isEmpty())
                    <div class="bg-white rounded-lg shadow-sm p-6 text-center">
                        ...
                        <p class="mt-2 text-gray-500">Tidak ada rekam medis yang tersedia.</p>
                    </div>
                @else
                    <div class="grid grid-cols-1 gap-6">
                        <div class="max-h-[900px] overflow-y-auto">
                        @foreach ($servicesGroups as $servicesGroup)
                            @if ($servicesGroup->diagnosis && $servicesGroup->diagnosis->medicalRecord)
                                <div class="medical-record-card bg-white rounded-lg shadow-sm overflow-hidden hover:bg-gray-50">
                                    <div class="p-6">
                                        <div class="flex items-center justify-between mb-4">
                                            <div class="flex items-center space-x-3">
                                                ...
                                                <div>
                                                    <p class="text-sm font-medium text-txt">Kode Diagnosa</p>
                                                    <p class="text-lg font-semibold text-gray-900">
                                                        {{ $servicesGroup->diagnosis->diagnosis_code }}</p>
                                                </div>
                                            </div>
                                            <div class="text-right">
                                                <p class="text-sm font-medium text-txt">Tanggal Diagnosa</p>
                                                <p class="text-lg font-semibold text-gray-900">
                                                    {{ \Carbon\Carbon::parse($servicesGroup->diagnosis->diagnosis_date)->format('d M Y') }}
                                                </p>
                                            </div>
                                        </div>
                                        ...
                                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                                            <div>
                                                <h3 class="text-sm font-medium text-txt mb-2">Riwayat Penyakit</h3>
                                                <p class="text-gray-900">{{ $servicesGroup->diagnosis->riwayat_penyakit }}
                                                </p>
                                            </div>
                                            <div>
                                                <h3 class="text-sm font-medium text-txt mb-2">Tindakan Medis</h3>
                                                <p class="text-gray-900">{{ $servicesGroup->diagnosis->tindakan_medis }}</p>
                                            </div>
                                        </div>

                                        <div class="mt-6">
                                            <h3 class="text-sm font-medium text-txt mb-2">Deskripsi</h3>
                                            <p class="text-gray-900">{{ $servicesGroup->diagnosis->description }}</p>
                                        </div>
                                    </div>
                                </div>
                            @endif
